...
{{^ ishControlsHide.views-all }}
  <a href="#" class="sg-acc-handle sg-control-trigger sg-icon sg-icon-eye" id="sg-t-toggle" title="${t("View")}"><span class="visually-hidden">${t("View")}</span></a>
  <ul class="sg-acc-panel sg-right sg-checklist" id="sg-view">
    ...
    {{^ ishControlsHide.views-new }}<li><a href="#" target="_blank" id="sg-raw" class="sg-checklist-icon sg-icon sg-icon-link">${t("Open in new tab")}</a></li>{{/ ishControlsHide.views-new }}
  </ul>
{{/ ishControlsHide.views-all }}
`
  }
